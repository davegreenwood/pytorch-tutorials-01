...
from dg_examples.models import MLPClassifier
...
model = MLPClassifier(xdim=XDIM, nclasses=NCLASS).to(DEVICE)
...
WRITER = SummaryWriter(os.path.expanduser("~/runs/mnist/logits"))
# in a terminal run:
# tensorboard --logdir=~/runs/mnist/logits
...
for epoch in range(EPOCHS):
    ...
    correct, total = accuracy(test, model, DEVICE)
...
torch.save(model, SAVE + "mnist_classifier_00.pkl")
